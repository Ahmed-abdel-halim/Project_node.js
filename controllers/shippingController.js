...
exports.createShippingOption = (req, res) => {
  ...
  const sql = 'INSERT INTO shipping_options (name, description, price, estimated_days) VALUES (?, ?, ?, ?)';
  
  db.query(sql, [name, description, price, estimated_days], (err, result) => {
    if (err) return res.status(500).json({ error: err.message });

    res.status(201).json({ message: 'Shipping option created', id: result.insertId });
  });
};
...
exports.updateShippingOption = (req, res) => {
  ...
  const sql = 'UPDATE shipping_options SET name = ?, description = ?, price = ?, estimated_days = ? WHERE id = ?';
